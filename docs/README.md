# Parbin documentation

Onboarding and reference material for humans and automation working on this repository.

| Document | Purpose |
| -------- | ------- |
| [Architecture](./architecture.md) | Monorepo layout, backend and frontend modules, data flow |
| [API](./api.md) | HTTP endpoints, JSON shapes, auth, errors |
| [Frontend & styling](./frontend-and-styling.md) | React stack, Tailwind/shadcn, tokens, where UI code lives |
| [Agents](./agents.md) | Notes for AI assistants and scripted tooling |

Project setup and environment variables: [README.md](../README.md).

Contributing: [CONTRIBUTING.md](../CONTRIBUTING.md). License: [LICENSE](../LICENSE).
